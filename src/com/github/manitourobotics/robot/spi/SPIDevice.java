...
package com.github.manitourobotics.robot.spi;
...
public class SPIDevice extends SensorBase {
    ...
    public static void initBus(final DigitalOutput clk, final DigitalOutput mosi, final DigitalInput miso) {
        if (spi == null)
            ...
            throw new SPIDevice.BadSPIConfigException("The SPI bus can only be initialized once");
    ...
    public long[] transfer(long[] writeValues, int[] numBits) {
        if (writeValues.length != numBits.length) {
            throw new SPIDevice.BadSPIConfigException("The number of values to write does not match array of data lengths");
        }
        for (int i = 0; i < numBits.length; ++i) {
            if (numBits[i] < 1 || numBits[i] > 32) {
                throw new SPIDevice.BadSPIConfigException("All values in the data length must be >0 and <=32");
            }
        }
...
//            tSPI.writeConfig_LatchFirst(false);
            ...
            //cs.set(!csActiveHigh);
            //JROSS these two lines cause a short chip select on non-default clock rates
            //(possibly only in write only mode
            //commenting these out might break having multiple devices on the bus.
//            tSPI.writeChannels_SS_Module(0);  
//            tSPI.writeChannels_SS_Channel(0);
        }
        return readValues;
    ...
    public final void setClockRate(double hz) {
        ...
        if (hz <= 76628.4) {
            ...
            throw new SPIDevice.BadSPIConfigException("Clock Rate too high. Hz: " + hz);
        }
        if (delay > 255) {
            throw new SPIDevice.BadSPIConfigException("Clock Rate too low. Hz: " + hz);
        }
